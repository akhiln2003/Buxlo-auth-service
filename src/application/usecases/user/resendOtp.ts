...
export class ResendOtpUseCase implements IresendOtpUseCase {
    constructor( private redisRepository: IredisRepository , private otpService: IOtpService ){}
    async execute(user: Pick<User, "email" >): Promise<string | void> {
        try {
            
            const isUnverifide = await this.redisRepository.getUnverifiedUser(user.email);
            if( !isUnverifide ) {
